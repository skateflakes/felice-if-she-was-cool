...
class KickBanMixin(MixinMeta):
    ...
    async def ban_user(
        ...
        if removed_temp:
            ...
            user_handle = str(user) if isinstance(user, discord.abc.User) else "(Username couldn't be found)"
            try:
                ...
                success_message = _("✅ Banned `{user_handle} ({user_id})` from the server.").format(user_handle=user_handle, user_id=user.id)
